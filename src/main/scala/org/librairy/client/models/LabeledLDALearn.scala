package org.librairy.client.models
...
/**
  ...
  */
object LabeledLDALearn {


  def apply(filePath: String, domain: String, iterations: Int): Boolean = {



    val source = CSVFile(filePath) ~> IDColumn(1);
    ...
    TermMinimumDocumentCountFilter(2)     // filter labels in < 10 docs
  }
    ...
    val modelPath = file("models/llda/"+domain);

    // Trains the model, writing to the given output path
    TrainCVB0LabeledLDA(modelParams, dataset, output = modelPath, maxIterations = iterations);
